				...
				try{
					Ext.Ajax.request({
						...
						success: function(response){
							...
							if(detail.length == 0){
								if(node == null){
									var win = new qsOSupplierWin({contract_code:record.get('quotationCode'),
										ownContactPerson:record.get('ownContactPerson'),
										memo:memo,
										customerCode:customerCode,
										customerName:customerName});
								}else{
									var win = new qsOSupplierWin({quotationId:record.get('id'),
										contract_code:record.get('quotationCode'),
							...
							var conEditWin = new Ext.ffc.ContractOutStockEditWindow(
								{
									outStockInfor:{quotationId:contractInforId,outStockType:1,status:0,quotationCode:quotationCode,outStockDetails:[],memo:memo},
									listeners :{
										close : function(p){
											var win = new qsOSupplierWin({quotationId:record.get('id'),
												contract_code:record.get('quotationCode'),
							...
							try{
								Ext.Ajax.request({
										...
										success: function(response){
											...
											var conEditWin = new Ext.ffc.ContractOutStockEditWindow(
												{
													outStockInfor:{quotationId:contractInforId,outStockType:5,status:0,quotationCode:quotationCode,outStockDetails:detail,memo:memo},
													listeners :{
														close : function(p){
															if(node == null){
