	...
	var pOSupplerGrid = Ext.extend(Ext.grid.GridPanel,{
		constructor : function(_cfg){
			...
			Ext.apply(this, _cfg);
			var proxy = new Ext.data.HttpProxy({url: PATH + "/planOrder/suppliersInforList.do?planId=" + _cfg.planId});
...
var  pOSupplierSelectForm = Ext.extend(Ext.FormPanel, {
		...
		pOSupplierSelectForm.superclass.constructor.call(this, {
	        ...
	        items:[
	           ...
	           items:[
				  ...
				  {items: [{xtype:'textfield',fieldLabel: '品牌',name: 'brand',anchor:'90%'},{xtype:'hidden',fieldLabel: 'planId',name: 'planId',value:_cfg.planId}]}
	           ]//items
	          }
	        ],
			keys : {
	...
	var pOsupplierWin = Ext.extend(Ext.Window,{
		...
		constructor : function(_cfg){
			...
			Ext.apply(this, _cfg);
			this.selectForm2 = new pOSupplierSelectForm({planId:_cfg.planId});
			this.supplier_grid = new pOSupplerGrid({planId:_cfg.planId});
			...
			this.supplier_grid.on('rowdblclick',function(grid,rowIndex,e)
			{
				...
				win.on('show',function(){
					Ext.Ajax.request({
						method: "post",
						params: { 'supplierId' : record.get('id'),'planId':_cfg.planIdArr},
						url: PATH + '/planOrder/ConsultPlanInforsAction.do',
						success: function(response){
							//try{
								eval("var orderDetail=" + response.responseText);
								win.nav2.addGrid.getForm().loadRecord(contractOrderRecord);
								win.nav2.addGrid.taxRateCombox.store.load();
								
								win.nav1.addTree.getStore().loadData({totalProperty:orderDetail.length,root:orderDetail});
							//}catch(e){alert("sss:"+e);}
						}
					});
