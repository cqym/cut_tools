	...
	var pOAddTree =  Ext.extend(Ext.grid.EditorGridPanel,{
		...
		var Bill = Ext.data.Record.create([
										...
										{name: 'contractDeliveryDate', type: 'string',mapping:"contractDeliveryDate"},
...
//						return
//					}
						var productMoney = e.record.get('orderAmount')*e.record.get('price')*1==0?e.record.get('orderAmount')*e.record.get('price')*1:(e.record.get('orderAmount')*e.record.get('price')*1).toFixed(2);
					e.record.set('productMoney',productMoney);
		...
		constructor : function(_cfg){
			...
			var config = [
				//
					...
					{xtype:'numberfield',name: 'overallRebate',emptyText : 0, readOnly : this.isReadOnly,value:0,
							allowDecimals : false,
							allowNegative : false,
							decimalPrecision : 0,
							minValue : 0,
...
//						modal:true,
						...
						buttons : [{
							...
							handler : function() {
								...
								Ext.apply(record.data,{'currencyId':this.nav2.addGrid.currCombox.curid});
