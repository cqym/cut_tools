	...
	var win = new Ext.ffc.purchaseProductsWindow({
		...
		callBackMethod:function(reArr,win){
				for(var i=0;i<reArr.length;i++){
						...
						'deliveryDate':'','memo':'','memo2':'','contractDeliveryDate':''
						};
...
/**合同订单详细grid**/
	Ext.ffc.PurchaseOrder.PurchaseOrderDetail =  Ext.extend(Ext.grid.EditorGridPanel,{
		...
		if(isContractOrder(this.orderInfor.orderType)){
          this.bbar  = [{xtype:'label',html : "销售合同交货地点及运输方式:<font color = 'red'>"+this.orderInfor.contractDeliveryAddress+"</font>,<font color='green'>"+this.orderInfor.contractTrafficMode+"</font>"}];
		}
