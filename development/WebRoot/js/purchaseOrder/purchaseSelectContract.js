...
Ext.ffc.PurchaseOrder.cOContractWin = Ext.extend(Ext.Window,{
		...
		constructor : function(_cfg){
			...
			this.contract_list.on('rowdblclick',function(grid,rowIndex,e){//双击合同
				...
				this.callBackMethod(record.get("id"),record.get("customerCode"),record.get("contractCode"));
				
			},this);
			
			this.cOContractTree.on('rowdblclick',function(grid,rowIndex,e) {//双击合同明细
				var r = grid.getStore().getAt(rowIndex);
				this.callBackMethod(record.get("id"),record.get("customerCode"),record.get("contractCode"),r.get('productBrand'));
			},this);
