        ...
        columns: [
			...
			{header: "货款回收情况", width: 100,  dataIndex: 'auditInfor',
				renderer:function(value, cellmeta, record, rowIndex, columnIndex, store){
					...
					var str = "<a href=\"javascript:Ext.ffc.showContractAccountInforWin('"+data.id+"','"+data.contractCode+"','"+data.customerName+"','"+data.customerCode+"','"+data.finalMoney+"',"+_config.isReturnMoneyHide+")\">货款回收</a>";
					return str;
				}
			},
			{header: "票据开据情况", width: 100,  dataIndex: 'auditInfor',
				renderer:function(value, cellmeta, record, rowIndex, columnIndex, store){
					...
					var s = '';
					if(invoiceStatus == null || invoiceStatus * 1 == 0){
					    text = '未开发票';
					}else if(invoiceStatus * 1 == 1){
						text = '部分已开';
					}else{
						text = '已全开';
						s = "color:#33FFFF";
