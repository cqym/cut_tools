		...
		constructor : function(_cfg){
			...
			this.contract_list.on('rowdblclick',function(grid,rowIndex,e)
			{
				...
				try{
						Ext.Ajax.request({
							method: "post",
							params: { 'quotationId' : contractInforId},
							...
							success: function(response){
									...
									if(detail.length == 0){
										...
										var win = new qOSupplierWin({quotationId:contractInforId,deliveryAddressType:record.get('deliveryType'),quotationId:record.get('id'),
											quotationCode:record.get('quotationCode'),ownContactPerson:record.get('userName'),customerName:customerName,customerCode:customerCode,memo:memo});
									...
									{
										outStockInfor:{quotationId:contractInforId,userName:LoginInfor.user.trueName,customerCode:customerCode,customerName:customerName,outStockType:5,status:0,quotationCode:quotationCode,outStockDetails:detail,memo:memo},
										listeners :{
											close : function(p){
							...
							try{
									Ext.Ajax.request({
										...
										success: function(response){
												...
												var conEditWin = new Ext.ffc.QuotationOutStockEditWindow(
												{
													outStockInfor:{quotationId:contractInforId,userName:LoginInfor.user.trueName,customerCode:customerCode,outStockType:5,status:0,quotationCode:quotationCode,outStockDetails:detail,memo:memo},
													nextStepButtonTitle: '直接采购',
													listeners :{
														close : function(p){
															var win = new qOSupplierWin({quotationId:contractInforId,deliveryAddressType:record.get('deliveryType'),
