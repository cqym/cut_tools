...
/**预定订单详细grid**/
	Ext.ls.scheduleOrder.addGrid =  Ext.extend(Ext.grid.EditorGridPanel,{
		...
		,'serialNumber','orderAmount','productMoney','deliveryDate','projectCode','memo','memo2','contractDeliveryDate']});
		/**给store传参**/
		...
		Ext.ls.scheduleOrder.addGrid.superclass.constructor.call(this, {
			...
			listeners : {
			    ...
			    if(e.field == 'memo' || e.field == 'memo2'){return ;}
				    if(e.field == 'orderAmount' &&  e.record.get('orderAmount')>e.record.get('remainAmount')+e.originalValue*1)
					{
										 ...
										 if(responseArray.success == true){
											 ...
											 Ext.Ajax.request({
												...
												success : function(_response) {
													var _responseArray = Ext.util.JSON.decode(_response.responseText); 
													 if(_responseArray.success == true){
														 Ext.Msg.show({title:'价格提示',msg:"序列号为:"+ _responseArray.numStr+"产品的采购价格高于合同中销售价格",buttons: Ext.Msg.OK,icon: Ext.MessageBox.INFO,width: 300});
													 }
											}})
