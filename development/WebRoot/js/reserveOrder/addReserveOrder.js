...
Ext.ls.reserveOrder.add = function(btn,e){
	...
	var vals = grid.ownerCt.ownerCt.form.getForm().getValues();
	var store = grid.store;
	var win = new selectRorderProductwindow({supplierId:vals.supplierId,
...
/**产品储备订单详细的store**/
		...
		fields : ['id','productName','brandCode','productCode','productUnit','orderAmount','productMoney',
			'productBrand','price','parentToolsId','toolsId','leaf','deliveryDate','memo']});
		/**给store传参**/
		if(this.URL)
		{
		...
		Ext.ls.reserveOrder.addGrid.superclass.constructor.call(this, {
			...
			columns:[
					...
					{header:'采购单价',width:60,dataIndex:'price',editor: new Ext.form.NumberField({
						   allowBlank: false,
						   allowNegative: false,
						   style: 'text-align:left',
						   gridObj:this,
...
//						return
//					}
					var productMoney = e.record.get('orderAmount')*e.record.get('price')*1;
					e.record.set('productMoney',productMoney==0?productMoney:productMoney.toFixed(2));
...
//					{xtype:'textfield',  name: 'status',value:'编制',readOnly : true,x:100,y:93,width:170},
					...
					//6
					 ...
					 {xtype:'textfield' ,name: 'effectConditions',x:630,y:212 ,width : 440, readOnly : this.isReadOnly,value:'本合同一式两份，卖方一份，买方一份，双方签字盖章后生效。', allowBlank : false,blankText:'该项为必填项!'},
					 //8
