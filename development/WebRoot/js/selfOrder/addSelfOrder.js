...
Ext.ls.selfOrder.deleteProduct = function(btn,e){
	...
	var handleDelete = function (btn){
		if(btn == 'ok') {	
			...
			var productMoney = prom==0?prom:prom.toFixed(2);
			var totalMoney = (prom*rat*1 + prom*1)==0?(prom*rat*1 + prom*1):(prom*rat*1 + prom*1).toFixed(2);
			var finalMoney = (totalMoney*1*(1- overallRebate/100))==0?(totalMoney*1*(1- overallRebate/100)):(totalMoney*1*(1- overallRebate/100)).toFixed(2);
			var taxMoney = (totalMoney - prom*1)==0?(totalMoney - prom*1):(totalMoney - prom*1).toFixed(2);
			grid.ownerCt.ownerCt.form.getForm().setValues({productMoney:productMoney});
			grid.ownerCt.ownerCt.form.getForm().setValues({totalMoney:totalMoney});
			grid.ownerCt.ownerCt.form.getForm().setValues({finalMoney:finalMoney});
			grid.ownerCt.ownerCt.form.getForm().setValues({taxMoney:taxMoney});
			Ext.Ajax.request({
...
Ext.ls.selfOrder.setupDate = function(btn,e){
	...
	menu["on"]("select", function(a,b){
		var deliveryDate = dataf.formatDate(dataf.parseDate(b));
		...
		constructor : function(_cfg){
			...
			Ext.ls.selfOrder.addGrid.superclass.constructor.call(this, {
					...
					columns:[
						...
						{header:'合同分项名称',width:180,dataIndex:'proSortName',disEnableEdit:true},
						{header:'项目',width:100,dataIndex:'projectCode',disEnableEdit:true},
						...
						{header:'合同剩余数量',width:80,dataIndex:'remainAmount',disEnableEdit:true},
						...
						{header:'采购交货期',width:100,dataIndex:'deliveryDate',disEnableEdit:true,
								listeners : {
									'click' : function() {
										try{
...
//					{xtype:'textfield',  name: 'status',value:'编制',readOnly : true,x:100,y:62,width:170},
					...
					//6
					 ...
					 {xtype:'textfield' ,name: 'mome', width : 440, readOnly : this.isReadOnly,x:100,y:122},
					 //hidden
	...
	Ext.ls.selfOrder.addWin = Ext.extend(Ext.Window,{
		...
		constructor : function(_cfg){
			...
			Ext.ls.selfOrder.addWin.superclass.constructor.call(this, {
						...
						maximizable:true,
						layout:"border",
						bbar :[{xtype:'label',html : "销售合同交货地点及运输方式:<font color = 'red'>"+this.deliveryAddressType+"</font>,<font color='green'>"+this.trafficMode+"</font>"}],  
						buttons : [{
							text : "保存",
							...
							handler : function() {
								...
								var minDeliveryDate = "3000-01-01";
								for(var i=0;i<childrenArray.length;i++)
								{
									...
									if(minDeliveryDate > nDate){
									    minDeliveryDate = nDate;
									}
								}

								var record = new Ext.data.Record(this.form.getForm().getValues());
									record.data["deliveryDate"] = minDeliveryDate;
...
//											 this.close();
							...
							handler : function() {
								...
								var record = new Ext.data.Record(this.form.getForm().getValues());
									Ext.apply(record.data,{'currencyId':this.form.currCombox.curid});
								var formV = this.form.getForm().getValues();
								if(!checkOrderInfor(formV)){return;}
								var ss = [];
...
//alert(ed.editNode.firstChild.cols['orderAmount']);
	...
	if(overallRebate == null)
	{
		...
		var finalMoney = (totalMoney*1*(1- overallRebate/100)) ==0?(totalMoney*1*(1- overallRebate/100)):(totalMoney*1*(1- overallRebate/100)).toFixed(2);
		var taxMoney = (totalMoney - prom*1)==0?(totalMoney - prom*1):(totalMoney - prom*1).toFixed(2);
		obj.corder_form.getForm().setValues({productMoney:productMoney,totalMoney:totalMoney,finalMoney:finalMoney});
		obj.corder_form.getForm().setValues({taxMoney:taxMoney});
	}
   ...
   this.show = function(){
       if(this.method != null){
	       this.method();
			...
			store.load();
			/**订单修改页面**/
			var win = new Ext.ls.selfOrder.addWin({orderId:this.id,detailFlag:true,URL:Ext.ls.selfOrder.updateUrl});
			/**设置窗口的标题**/
