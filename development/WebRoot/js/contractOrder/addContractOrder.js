...
/**合同订单详细grid**/
	Ext.ls.contractOrder.addGird =  Ext.extend(Ext.grid.EditorGridPanel,{
		...
		fields : ['id','brandCode','productCode','productName','productUnit','contractAmount',
			...
			'contractProjectSortId','proSortName','projectCode','serialNumber','orderAmount','productMoney','deliveryDate','memo','memo2','contractDeliveryDate']});
		/**给store传参**/
		...
		Ext.ls.contractOrder.addGird.superclass.constructor.call(this, {
			...
			columns:[
					...
					{header:'项目',width:50,dataIndex:'projectCode',sortable:true},
			...
			listeners : {
				...
				if(e.field == 'orderAmount' && e.record.get('orderAmount')>e.record.get('remainAmount')+e.originalValue)
					{
...
//					{xtype:'textfield',  name: '',value:'编制',readOnly : true,x:100,y:93,width:170},
	...
	Ext.ls.contractOrder.addWin = Ext.extend(Ext.ffc.AuditBusinessDetailWindow,{
		grid:null,
		form:null,
		...
		constructor : function(_cfg){
			...
			this.addSaveButtonHiden = true;
			if(this.openType == 'add'){
			  this.addSaveButtonHiden = false;	
			}
			this.updateSaveButtonHiden = true;
			if(this.openType == 'update'){
			  this.updateSaveButtonHiden = false;	
			}
			Ext.ls.contractOrder.addWin.superclass.constructor.call(this, {
				    renderTo: Ext.getBody(),
						...
						buttons : [{
							text : "保存",
							hidden:this.updateSaveButtonHiden,
							handler : function() {
   ...
   this.show = function(){
       if(this.method != null){
	    ...
	    var _this = this;
			...
			store.load();
			/**订单修改页面**/
			var win = new Ext.ls.contractOrder.addWin({orderId:this.id,detailFlag:true,URL:Ext.ls.contractOrder.detailUrl,auditType:_this.auditType,_grid:_this.grid,_id:_this.id});
			/**设置窗口的标题**/
