...
public class GeneralQuoToExcel {
	...
	private AccessoriesDto getAccDto(String busId, int busType) {
		...
		List<AccessoriesDto> list = accessoriesDao
				.getAccessoriesByBusId(paramMap);
	...
	private void insertImage(HSSFWorkbook sysEventWorkBook,
			HSSFSheet demoSheet, TCompanyInfor comInfo, int footRow) {
		...
		if (headerSlave != null) {
			...
			String headerFilePath = new StringBuffer(logoPath).append(
					headerPath).toString();
		...
		if (footerSlave != null) {
			...
			String footerFilePath = new StringBuffer(logoPath).append(
					footerPath).toString();
		...
		try {
			...
			HSSFClientAnchor anchor = new HSSFClientAnchor(0, 30, 131, 36,
					(short) 0, 0, (short) 11, 1);
			HSSFClientAnchor footAnchor = new HSSFClientAnchor(0, 100, 131, 36,
					(short) 0, footRow, (short) 11, footRow + 2);
			...
			if (file != null && file.canRead()) {
				...
				if ("png".equalsIgnoreCase(extName)) {
					...
					patriarch.createPicture(anchor, sysEventWorkBook
			...
			if (footFile != null && footFile.canRead()) {
				...
				if ("png".equalsIgnoreCase(footExtName)) {
					...
					patriarch.createPicture(footAnchor, sysEventWorkBook
							.addPicture(footerbyteArrayOut.toByteArray(),
									HSSFWorkbook.PICTURE_TYPE_PNG));
	...
	private void fillDataToCell(HSSFRow row, int cell, String value0,
			int cell2, String value1) {
		HSSFCell cell1 = row.getCell(cell) == null ? row.createCell(cell) : row
				.getCell(cell);
		cell1.setCellValue(value0);
		HSSFCell cell3 = row.getCell(cell2) == null ? row.createCell(cell2)
				: row.getCell(cell2);
		cell3.setCellValue(new BigDecimal(value1).doubleValue());
	}

	private void fillDataToCell(HSSFRow row, String value) {
		HSSFCell cell = row.getCell(8) == null ? row.createCell(8) : row
				.getCell(8);
	...
	public void fillDataToQuo(HSSFSheet demoSheet, QuotationDto quoPro) {
		...
		fillDataToCell(row2, quoPro.getQuotationCode(), quoPro
				.getCustomerName());
		...
		TExchangeRate expo = exchangeRateDAO.selectByPrimaryKey(quoPro
				.getCurrency());
	...
	private void createTableRow(QuotationDetailDto product, HSSFRow row,
			boolean hasMemo2, boolean hasMemo3) {
	...
	public void insertRow(List<QuotationDetailDto> quoProList, HSSFWorkbook wb,
			HSSFSheet sheet, int startRow) {
	...
	private void createMemoCell(HSSFWorkbook wb, HSSFSheet sheet,
			int cellIndex, String memo) {
	...
	private void createMemoLine(HSSFSheet sheet, int rowIndex, int cellInex) {
		HSSFRow rowLine = sheet.getRow(rowIndex) == null ? sheet
	...
	public void createCountInfo(HSSFSheet demoSheet, int rowIndex,
			QuotationDto quoPro) {
		int step = rowIndex;
		...
		if (quoPro.getOverallRebate() != null
				&& new BigDecimal(quoPro.getOverallRebate())
						.compareTo(BigDecimal.ZERO) != 0) {
			HSSFRow row5 = demoSheet.getRow((short) rebateRow);
			fillDataToCell(row5, 7, "整单折扣：", 8, new Double(Double
					.valueOf(quoPro.getOverallRebate()) / 100).toString());
			rebateRow += 1;
		}
	...
	public void createMemoInfo(HSSFWorkbook sysEventWorkBook,
			HSSFSheet demoSheet, int rowIndex, QuotationDto quoPro,
			String deliveryDate) {
		TCompanyInfor comInfo = tcompanyInforDAO.getCompanyByName(quoPro
				.getSellerName());
		...
		if (quoPro.getValidStartDate() != null
				&& quoPro.getValidEndDate() != null)
			setCellValue(validDateRow, 2, quoPro.getValidStartDate() + " ―― "
					+ quoPro.getValidEndDate());
	...
	public void exportExcel(OutputStream os,
			List<QuotationDetailDto> quoProList, QuotationDto quoPro)
			throws IOException {
		...
		for (int i = 0; i < sheetNum; i++) {
			...
			fillDataToQuo(childSheet, quoPro);
			insertRow(quoProList, wb, childSheet, 7);
			createCountInfo(childSheet, 9 + quoProList.size(), quoPro);
			...
			createMemoInfo(wb, childSheet, 9 + quoProList.size(), quoPro,
					deliveryDate);
